...
<ParamField body="signal_timeframe_bucket" type="integer" default="5">
  Groups submitted time series into lower timeframe for signal/confirmation analysis.
...
df = pd.DataFrame({
    'Datetime': date_rng.dt.strftime("%Y-%m-%d %H:%M:%S"),
    'Close': close_prices,
