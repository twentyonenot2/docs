...
<Steps>
      ...
      <Step title="Signal time series">
        Agent creates new time series by aggregating original data. Aggregation period defined by client (e.g. 3-day if client specifies).
      </Step>
      <Step title="Analysis">
        Agent compares original time series with newly created signal series. Looks for periods where both show same directional volatility patterns.
      </Step>
