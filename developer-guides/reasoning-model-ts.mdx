...
<Note>
  AGNs can model how directional changes across multiple timeframes evolve into sustained trends, distinguishing structurally significant market moves from noise.
</Note>

<Tip>
  We recommend using higher frequency data compared to your target forecast horizon, as this leverages the architecture's propagation methodology effectively (e.g., use 15 or 30 min data to model daily moves).
</Tip>
...
AGNs are designed as a multi-timeframe network, using directional insights from multiple timeframes to form a holistic picture of market dynamics for the chosen security.
  ...
  <Accordion title="High-to-Low Frequency Propagation Assumption">
    The API assumes that all price movements originate in high-frequency time series before propagating into lower-frequency time series. Therefore, the API will only return directional insights that it believes will propagate into lower frequency timeframes.
  </Accordion>
  ...
  <Accordion title="Interpreting Trend Signals">
    Each directional signal should be interpreted within its trend context: the first signal in a consecutive trend marks the start of a trend, while consecutive forecasts in the same direction indicate the consistency of the current trend.
  </Accordion>
