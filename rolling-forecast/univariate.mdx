...
This function generates a rolling forecast that predicts the continuation of the current directional change over the selected timeframe, based on the subtle initial shift.
...
forecast = client.univariate_rolling_forecast(
...
"""
Automatically saves each time step's rolling forecast, appending the results to a single .csv file (e.g., './17102025.csv') using a 5000 period moving window.
"""
```